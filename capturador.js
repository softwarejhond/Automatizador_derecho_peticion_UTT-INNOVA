...
if (process.argv[2]) {
    ...
    if (!cedula || !curso || !moodleUser || !moodlePass) {
        console.error("\n❌ ERROR: Parámetros incompletos");
        console.error("Uso: node capturador.js <cedula> <curso> <email> <usuario> <contraseña>");
        process.exit(1);
    }
...
async function procesarUsuariosDelJSON() {
    console.log(`\n📋 Encontrados ${usuarios.length} usuarios para procesar\n`);
    ...
    for (let i = 0; i < usuarios.length; i++) {
        ...
        console.log(`\n${'='.repeat(60)}`);
        console.log(`⏳ Usuario ${numeroActual}/${usuarios.length}`);
